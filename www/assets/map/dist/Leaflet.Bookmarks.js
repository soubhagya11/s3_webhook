...
!function(e){if("object"==typeof exports&&"undefined"!=typeof module)module.exports=e();else if("function"==typeof define&&define.amd)define([],e);else{var f;"undefined"!=typeof window?f=window:"undefined"!=typeof global?f=global:"undefined"!=typeof self&&(f=self),(f.Leaflet||(f.Leaflet={})).Bookmarks=e()}}(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
  ...
  var Bookmarks = L.Control.extend( /**  @lends Bookmarks.prototype */ {
    ...
    options: {
       ...
       */
      storage: null,
      bookmark_name: 'leaflet-bookmarks',
      ...
      animateClass: 'bookmark-added-animation',
      animateDuration: 0,
      ...
      bookmarkTemplate: '<li class="{{ itemClass }}" data-id="{{ data.id }}">' +
        ...
        '<span class="{{ nameClass }}">{{ data.bookmark_name }}</span>' +
        '</li>',
      ...
      title: JSON.parse(localStorage.getItem('mapNameObj')).mapBookMarks,
      emptyMessage: JSON.parse(window.localStorage.getItem('mapNameObj')).nobookmarks,
      addBookmarkMessage: JSON.parse(window.localStorage.getItem('mapNameObj')).addbookmark,
       ...
       * @type {String}
       */
      popupTemplate: '<div class="bookmarkpopup-view"><h3>{{ bookmark_name }}</h3><p>{{ geo_coordinates }}</p></div>',
      ...
      getPopupContent: function(bookmark) {
        return substitute(this.options.popupTemplate, {
          // latlng: this.formatCoords(bookmark.latlng), 180828: To disable geocoordinates.
    ...
    initialize: function(options) {
      ...
      this._storage = options.storage ||
        (this.options.localStorage ?
          new Storage(this.options.bookmark_name, Storage.engineType.LOCALSTORAGE) :
          new Storage(this.options.bookmark_name, Storage.engineType.GLOBALSTORAGE));
    ...
    _createList: function(bookmarks) {
      ...
      // To display bookmarks title in bookmarks listing intially - 180828 (rushi)
      this._list.innerHTML = '<h5 class="bookmark-title"><b>'+mapNamesObj.mapBookMarks+'</b></h5>';
    ...
    _setEmptyListContent: function() {
      this._list.innerHTML = this._list.innerHTML + substitute(this.options.emptyTemplate,
    ...
    if (this._marker) {
      ...
      L.DomEvent.stop(evt);
      this.collapse();
      geo_coordinates = this._map.getCenter();
      var bookmarksdata = [];     
      this._storage.getAllItems(function(bookmarks) {
        for(var i = 0; i < bookmarks.length; i++){
          if((bookmarks[i].geo_coordinates[0]).toFixed(4) === geo_coordinates.lat.toFixed(4) && (bookmarks[i].geo_coordinates[1]).toFixed(4) === geo_coordinates.lng.toFixed(4)) {
            bookmarksdata.push(JSON.parse(JSON.stringify(bookmarks[i])));
            break;
          }
        }
      });
    ...
    _appendItems: function(bookmarks) {
      ...
      if (html !== '') {
        ...
        if (wasEmpty) {
            this._list.innerHTML = '<h5 class="bookmark-title"><b>'+mapNamesObj.mapBookMarks+'</b></h5>' + html;
        } else {
    ...
    _getBookmarkDataForTemplate: function(bookmark) {
        ...
        return {
          coords: this.formatCoords(bookmark.geo_coordinates),
          bookmark_name: this.formatName(bookmark.bookmark_name),
          zoom: bookmark.zoom,
          id: bookmark.id
          };
        }
    ...
    /**
     * @param  {L.LatLng} geo_coordinates
     * @return {String}
     */
    formatCoords: function(geo_coordinates) {
      if (this.options.formatCoords) {
        return this.options.formatCoords.call(this, geo_coordinates);
      } else {
        return geo_coordinates[0] + ',&nbsp;' + geo_coordinates[1];
      }
    },
     ...
     * @return {String}
     */
    formatName: function(bookmark_name) {
      if (this.options.formatName) {
        return this.options.formatName.call(this, bookmark_name);
      } else {
    ...
    _onBookmarkAddStart: function(evt) {
      ...
      this._marker = new L.Marker(evt.geo_coordinates, {
    ...
    _onBookmarkAdd: function(bookmark) {
      ...
      bookmark.floor_name = window.localStorage.getItem('fcurrentfloorname');
      var enterpriseObj = JSON.parse(window.localStorage.getItem('selected_enterprise_obj'));
      bookmark.enterprise = {
        'enterprise_id': parseInt(window.localStorage.getItem('selected_enterprise'), 10),
        'enterprise_name': window.localStorage.getItem('selected_enterprise_name'),
        'short_name': enterpriseObj.short_name
      }
      bookmark.user_profile = {
        'user_profile_id': parseInt(window.localStorage.getItem('login_user_id'), 10),
        'f_name': window.localStorage.getItem('f_name'),
        'l_name': window.localStorage.getItem('l_name')
      }
      // bookmark.userId = window.localStorage.getItem('login_user_id');
      var deviceId = window.localStorage.getItem('bookmarkDeviceId');
      bookmark.device_id = deviceId ? deviceId : '';
      bookmark.is_deleted = false;
      this._storage.getAllItems(function(bookmarkslist) {
        for (var i = 0, len = bookmarkslist.length; i < len; i++) {
          var bookmark_name = bookmark.bookmark_name.toLowerCase().trim().replace(/  +/g, ' ');
          if (bookmarkslist[i].bookmark_name.toLowerCase().trim().replace(/  +/g, ' ').match(bookmark_name)) {
            self._showAlert();
            unique = true;
            break;
          } 
  ...
  _onBookmarkEdited: function(evt) {
    ...
    this._storage.getAllItems(function(bookmarks) {
      ...
      for (var i = 0, len = bookmarks.length; i < len; i++) {
        var bookmarkname = bookmark.bookmark_name.toLowerCase().trim().replace(/  +/g, ' ');
        if (bookmarks[i].id !== bookmark.id && bookmarks[i].bookmark_name.toLowerCase().trim().replace(/  +/g, ' ') === (bookmarkname)) {
          self._showAlert();
          unique = true;
          break;
        } 
    ...
    if(unique === false) {
        this._storage.updateItem(bookmark.id, bookmark, function(item) {
          map.fire('bookmark:saved', {
          data: item
          });     
          var data = self._removeDuplicates(self._data, "id");
          self._data = [];
              for (var i = 0, len = data.length; i < len; i++) {
              if (data[i].id === bookmark.id) {
              data.splice(i, 1, bookmark);
  ...
  // 180828: Duplicates
  _removeDuplicates: function( arr, prop ) {
    var obj = {};
    ...
    _cleanBookmark: function(bookmark) {
      if (!L.Util.isArray(bookmark.geo_coordinates)) {
        bookmark.geo_coordinates = [bookmark.geo_coordinates.lat, bookmark.geo_coordinates.lng];
      }
  
      return bookmark;
    },
    // / To show uniqueness validation message
    _showAlert: function() {
      document.getElementById('bookmarkId').style.display = 'block';
    ...
    // To hide uniqueness validation message
    hideAlert: function() {
    ...
    _bookmarkOptions: function(bookmark) {
      var coords = L.latLng(bookmark.geo_coordinates);
    ...
    _gotoBookmark: function(bookmark) {
      this._map.setView(bookmark.geo_coordinates, bookmark.zoom);
      this._showBookmark(bookmark);
    },
    ...
    _removeBookmark: function(bookmark) {
      var remove = function(proceed) {
        ...
        this._storage.removeItem(bookmark.id, bookmark, function(bookmark) {
          self._onBookmarkRemoved(bookmarkValue);
        });
      }.bind(this);
    ...
    _onBookmarkRemoved: function(bookmark) {
      ...
      if (li) {
        ...
        self._storage.getAllItems(function(bookmarks) {
          if(bookmarks.length === 0) {
            self._setEmptyListContent();
          }
        });
    ...
    bookmarkToFeature: function(bookmark) {
      ...
      delete bookmark.geo_coordinates;
    ...
    _getBookmarkCoords: function(bookmark) {
      if (bookmark.geo_coordinates instanceof L.LatLng) {
        return [bookmark.geo_coordinates.lat, bookmark.geo_coordinates.lng];
      } else {
        return bookmark.geo_coordinates.reverse();
      }
    },
    ...
    fromGeoJSON: function(geojson) {
      ...
      for (var i = 0, len = geojson.features.length; i < len; i++) {
        ...
        if (!bookmark.properties.divider) {
          bookmark.properties.geo_coordinates = bookmark.geometry
  ...
  var FormPopup = L.Popup.extend( /** @lends FormPopup.prototype */ {
    ...
    options: {
      ...
      templateOptions: {
        ...
        inputPlaceholder: mapNamesObj.bookmark_name,
        ...
        editMenuText: mapNamesObj.edit,
        removeMenuText: mapNamesObj.remove,
        cancelMenuText: mapNamesObj.cancel,
        submitTextCreate: '+',
        submitTextEdit: '<span class="icon-checkmark"></span>',
        error: mapNamesObj.bookmark_already_exist
      },
      generateNames: false,
      minWidth: 160,
      generateNamesPrefix: mapNamesObj.bookmark+' ',
      template: '<form class="{{ formClass }}">' +
        '<div class="input-group"><input type="text" bookmark_name="bookmark-name" ' +
        'placeholder="{{ inputPlaceholder }}" class="{{ inputClass }}" value="{{ bookmark_name }}">' +
        '<input type="hidden" class={{ idInputClass }} value="{{ id }}">' +
        '<button type="submit" class="input-group-addon {{ submitClass }}">' +
        '{{ submitText }}</button></div>' +
        '<div class="{{ coordsClass }}">{{ coords }}</div>' + '<div id="bookmarkId" class="text-danger  text-center" style="display:none">{{ error }}</div>' +
        '</form>',
      menuTemplate: '<ul class="nav {{ mode }}" role="menu">' +
        '<li class="{{ editClass }}"><a href="#" class="{{ menuItemClass }}"><i class="fas fa-edit"></i>{{ editMenuText }}</a></li>' +
        '<li class="{{ removeClass }}"><a href="#" class="{{ menuItemClass }}"><i class="far fa-trash-alt"></i>{{ removeMenuText }}</a></li>' +
        '<li class="{{ cancelClass }}"><a href="#" class="{{ menuItemClass }}"><i class="fas fa-times"></i>{{ cancelMenuText }}</a></li>' +
        '</ul>'
    },
    ...
    _initLayout: function() {
      ...
      if (this.options.mode === modes.SHOW &&
        (this._bookmark.is_editable || this._bookmark.is_removable)) {
    ...
    _updateContent: function() {
      ...
      if (this.options.mode === modes.SHOW) {
        ...
        var modeClass = [];
        if (this._bookmark.is_editable) {
          modeClass.push(this.options.templateOptions.editableClass);
        }
        if (this._bookmark.is_removable) {
        ...
        content = substitute(template,
          L.Util.extend({}, this._bookmark || {}, this.options.templateOptions, {
            ...
            // coords: this.formatCoords( 180828: To disable geocoordinates.
    ...
    _getBookmarkData: function() {
      if (this.options.getBookmarkData) {
        ...
        return {
          geo_coordinates: this._source.getLatLng(),
          zoom: this._map.getZoom(),
          bookmark_name: input.value,
   ...
   * @param {Function} callback
   */
  GlobalStorage.prototype.updateItem = function(key, item, callback) {
    data[this._prefix + key] = item;
    callback(item);
  };
   ...
   * @param  {Function} callback
   */
  GlobalStorage.prototype.removeItem = function(key, item, callback) {
    var self = this;
   ...
   * @param {String} bookmark_name
   */
  var Storage = function(bookmark_name, engineType) {
  
    if (typeof bookmark_name !== 'string') {
      engineType = bookmark_name;
      bookmark_name = unique();
    }
  
    /**
     * @type {String}
     */
    this._name = bookmark_name;
   ...
   * @param {Function} callback
   */
  Storage.prototype.updateItem = function(key, item, callback) {
    this._engine.updateItem(key, item, callback);
    return this;
  };
   ...
   * @param  {Function} callback
   */
  Storage.prototype.removeItem = function(key, item, callback) {
    ...
    for (var key in this._storage) {
      if (this._storage.getItem(key) !== null &&
        ...
        this.getItem(key.substring(prefixLength), function(item) {
          if(item.floorName === window.localStorage.getItem('fcurrentfloorname')) {
            items.push(item);
          }
        });
      }
   ...
   * @param  {Function} callback
   */
  LocalStorage.prototype.removeItem = function(key, item, callback) {
    var self = this;
    this.getItem(key, function(item) {
   ...
   * @param  {Function} callback
   */
  LocalStorage.prototype.updateItem = function(key, item, callback) { console.log("update");
    var itemStr = item.toString();
    if (itemStr === '[object Object]') {
      itemStr = JSON.stringify(item)
    }
  ...
  function substitute(str, object, regexp) {
    return str.replace(regexp || (/{{([\s\S]+?)}}/g), function(match, bookmark_name) {
      bookmark_name = trim(bookmark_name);
  
      if (bookmark_name.indexOf('.') === -1) {
        if (match.charAt(0) == '\\') {
          return match.slice(1);
        }
        return (object[bookmark_name] != null) ? object[bookmark_name] : '';
  
      } else { // nested
        var result = object;
        bookmark_name = bookmark_name.split('.');
        for (var i = 0, len = bookmark_name.length; i < len; i++) {
          if (bookmark_name[i] in result) {
            result = result[bookmark_name[i]];
          } else {
            return '';
          }
        }
